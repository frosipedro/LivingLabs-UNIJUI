...
function ContatoPage() {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    toast.success("Mensagem enviada!", { description: `Obrigado, ${data.nome}. Responderemos em breve.` });
    reset();
  };

  return (
    <section className="container-page pt-20 md:pt-28 pb-24">
      <SectionReveal><span className="eyebrow">Contato</span></SectionReveal>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div className="grid sm:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <Input id="email" type="email" placeholder="voce@dominio.com" {...register("email")} aria-invalid={!!errors.email} />
                {errors.email && <p className="text-xs text-destructive">{errors.email.message}</p>}
              </div>
            </div>
            <div className="space-y-2">
              <Label htmlFor="assunto">Assunto</Label>
              <Input id="assunto" placeholder="Sobre o que você quer falar?" {...register("assunto")} aria-invalid={!!errors.assunto} />
              {errors.assunto && <p className="text-xs text-destructive">{errors.assunto.message}</p>}
            </div>
            <div className="space-y-2">
              <Label htmlFor="mensagem">Mensagem</Label>
              <Textarea id="mensagem" rows={6} placeholder="Escreva sua mensagem…" {...register("mensagem")} aria-invalid={!!errors.mensagem} />
            ...
            <Button type="submit" disabled={isSubmitting} className="rounded-full px-6">
              {isSubmitting ? "Enviando…" : (<>Enviar mensagem <Send className="size-4 ml-1.5" /></>)}
            </Button>
          </form>
        </SectionReveal>
          ...
          <aside className="rounded-2xl border border-border bg-card p-8 space-y-6">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">Rua do Comércio, 3000<br />Ijuí — RS, Brasil</p>
              </div>
            </div>
            <div className="flex items-start gap-4">
