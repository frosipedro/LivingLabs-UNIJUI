...
import { getProjectBySlug, getMemberById, LAB_LABEL, PROJECT_STATUS_LABEL, type Project } from "@/content/api";
          ...
          <div className="lg:col-span-7">
            <SectionReveal>
              <div className="flex flex-wrap items-center gap-3 text-sm">
                ...
                <span className="text-muted-foreground">· {PROJECT_STATUS_LABEL[project.status]}</span>
              </div>
            </SectionReveal>
            ...
            <SectionReveal delay={200}>
              <p className="mt-6 max-w-3xl text-xl text-muted-foreground leading-relaxed">{project.excerpt}</p>
            </SectionReveal>
          </div>
            ...
            <SectionReveal>
              <div className="grid gap-10">
                <section>
                  ...
                  <p className="mt-3 max-w-3xl text-lg text-foreground/90 leading-relaxed">{project.challenge}</p>
                </section>

                <section>
                  <h2 className="eyebrow">Solução</h2>
                  <p className="mt-3 max-w-3xl text-lg text-foreground/90 leading-relaxed">{project.solution}</p>
                </section>

                <section>
                  <h2 className="eyebrow">Resultados</h2>
                  <p className="mt-3 max-w-3xl text-lg text-foreground/90 leading-relaxed">{project.results}</p>
                </section>
              </div>
            </SectionReveal>
          </div>
              ...
              <div>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {project.categories.map((category) => (
                    <span key={category} className="px-3 py-1 text-xs font-medium rounded-full border border-border">{category}</span>
                  ))}
                </div>
              </div>
            </SectionReveal>
